...
export default function StandingsTableClient({ id, enriched, totalTeams, remainingMatches, journees, header }: Props) {
  ...
  useEffect(() => {
    ...
    if (hasOverrides) {
      ...
      fetch(`${process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:8080"}/api/competitions/${id}/simulate`, {
        method: "POST",
      ...
      fetch(`${process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:8080"}/api/competitions/${id}/projections?${params}`, { signal: abortRef.current.signal })
        .then((r) => r.json())
        .then((data: ProjectionResult[]) => setResults(data))
        .catch(() => {})
        .finally(() => setLoading(false));
